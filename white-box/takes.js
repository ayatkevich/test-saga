...
const testerFn = (helperName, effCreatorName) => (value, buffer) => ({
  value: step = {}
} = {}) => {
  const pattern =
    R.is(Object, value) && !R.is(Array, value) ? value.type : value || '*';
  assert(
    pattern,
    `If you pass an object to the "${helperName}" helper, ` +
  ...
  assert(step[sagaSignature], assertionMessage(expectedEff(), []));

  const {name: effName, eff} = retrieveEff(step);
  const err = assertionMessage(
    expectedEff(`${effCreatorName}${eff.maybe ? 'Maybe' : ''}`),
    prepareEff(step)
  );
  ...
  if (R.is(Array, eff.pattern)) {
    if (R.is(Array, pattern)) {
      ...
      assert(
        R.contains(pattern, eff.pattern),
